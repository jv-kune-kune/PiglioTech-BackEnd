...
public class UserController {
  ...
  @GetMapping()
  public ResponseEntity<List<User>> getUsersByRegion(@RequestParam(required = false) Region region,
      @RequestParam(required = false) String exclude) {
    if (region != null) {
      if (exclude != null) {
        return new ResponseEntity<>(userService.getUsersByRegionExclude(region, exclude),
            HttpStatus.OK);
      } else {
  ...
  public ResponseEntity<User> saveUser(@RequestBody User user) {
    if (user.getUid() == null || user.getName() == null || user.getEmail() == null
        || user.getRegion() == null || user.getThumbnail() == null) {
  ...
  public ResponseEntity<Void> removeBookFromUser(@PathVariable String id,
  ...
  @PatchMapping("/{uid}")
  public ResponseEntity<Map<String, Object>> updateUserDetails(@PathVariable String uid,
