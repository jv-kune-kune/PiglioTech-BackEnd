...
public class SwapServiceImpl implements SwapService {
  ...
  public List<MatchDto> getMatches(String userId) {
    return matchRepository.findMatchesByUserUid(userId).stream().filter(m -> {
      if (m.getUserOne().getUid().equals(userId)) {
        return !m.isUserOneDismissed();
      } else {
        return !m.isUserTwoDismissed();
      }
    }).map(SwapConverters::matchToDto).toList();
  }

  @Override
  public SwapRequest makeSwapRequest(SwapRequestDto dto) {
    List<Match> existingMatches = matchRepository.findMatchesBetweenUsers(dto.initiatorUid(), dto.receiverUid());
    if (!existingMatches.isEmpty()) {
      throw new EntityExistsException("Match already exists between users");
    }
      ...
      for (SwapRequest swap : existingSwaps) {
        ...
        if (swap.getInitiator().getUid().equals(dto.receiverUid())
          ...
          Match match = new Match(swap.getReceiver(), swap.getInitiator(), swap.getReceiverBook(),
    ...
    public void dismissSwap (SwapDismissal dismissal){
      ...
      if (match.isUserOneDismissed() && match.isUserTwoDismissed()) {
        List<SwapRequest> swapRequests = swapRequestRepository
                .findSwapRequestsBetweenUsers(match.getUserOne().getUid(), match.getUserTwo().getUid());
        for (SwapRequest swap : swapRequests) {
          if (swap.getReceiver().equals(match.getUserOne())
                  && swap.getReceiverBook().equals(match.getUserOneBook())
                  || swap.getReceiver().equals(match.getUserTwo())
                  && swap.getReceiverBook().equals(match.getUserTwoBook())) {
