...
class SwapControllerTest {
  ...
  void test_getSwaps_validId() throws Exception {
    MatchDto dto = new MatchDto(1L,
        ...
        new Book("54321", "TITLE 2", "AUTHOR 2", "1900", "http://thumbnail.com/4",
            "Two peas in a pod"));

    when(mockSwapService.getMatches(anyString())).thenReturn(List.of(dto));

    mockMvcController.perform(MockMvcRequestBuilders.get(endpoint).param("userId", "UID_2"))
  ...
  void test_postSwaps_conflict() throws Exception {
    ...
    mockMvcController.perform(MockMvcRequestBuilders.post(endpoint)
        .contentType(MediaType.APPLICATION_JSON).content(dtoBody))
  ...
  void test_createSwap_invalidFields() throws Exception {
    ...
    mockMvcController
        .perform(MockMvcRequestBuilders.post("/api/v1/swaps")
            .contentType(MediaType.APPLICATION_JSON).content(invalidDtoJson))
        .andExpect(MockMvcResultMatchers.status().isBadRequest());
  }
  ...
  void test_createSwap_duplicateRequest() throws Exception {
    ...
    mockMvcController
        .perform(MockMvcRequestBuilders.post("/api/v1/swaps")
            .contentType(MediaType.APPLICATION_JSON).content(duplicateDtoJson))
        .andExpect(MockMvcResultMatchers.status().isConflict());
  }
  ...
  void test_createSwap_valid() throws Exception {
    ...
    when(mockSwapService.makeSwapRequest(any(SwapRequestDto.class))).thenAnswer(a -> {
      SwapRequest request = new SwapRequest(
          new User("UID_1", "NAME 1", "EMAIL 1", Region.NORTH_WEST, "http://thumbnail.com/1"),
          new User("UID_2", "NAME 2", "EMAIL 2", Region.NORTH_WEST, "http://thumbnail.com/2"),
          new Book("ISBN_123", "TITLE 1", "AUTHOR 1", "2000", "http://thumbnail.com/book1",
              "Description"));
      request.setId(1L);
      return request;
    });
    ...
    mockMvcController
        .perform(MockMvcRequestBuilders.post("/api/v1/swaps")
            .contentType(MediaType.APPLICATION_JSON).content(validDtoJson))
        .andExpect(MockMvcResultMatchers.status().isCreated())
        .andExpect(MockMvcResultMatchers.jsonPath("$.id").exists())
        .andExpect(MockMvcResultMatchers.jsonPath("$.initiator").exists())
        .andExpect(MockMvcResultMatchers.jsonPath("$.receiver").exists())
  ...
  void test_postDismiss_validDismissal() throws Exception {
    ...
    mockMvcController
        .perform(MockMvcRequestBuilders.post(endpoint + "/dismiss")
            .contentType(MediaType.APPLICATION_JSON).content(dismissalBody))
